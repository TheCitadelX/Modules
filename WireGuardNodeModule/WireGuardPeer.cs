...
internal sealed class WireGuardPeer
{
    ...
    public static WireGuardPeer From(UserEntity user, JsonObject? template)
    {
        var resources = ParseObject(user.ResourceAllocationJson);
        var publicKey = GetString(template, "publicKey");
        if (string.IsNullOrWhiteSpace(publicKey))
        {
            throw new InvalidOperationException("WireGuard user template must contain publicKey.");
        }

        var allowedIps = GetString(template, "allowedIps")
            ?? GetString(resources, "peerAddress")
            ?? throw new InvalidOperationException("WireGuard user requires an allocated peerAddress.");
