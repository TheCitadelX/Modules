...
internal sealed class AmneziaWGConfigDocument
{
    private readonly List<string> _lines;

    private AmneziaWGConfigDocument(List<string> lines)
    {
        _lines = lines;
    }

    public static AmneziaWGConfigDocument Load(string path)
    {
        if (!File.Exists(path))
        {
            throw new FileNotFoundException("AmneziaWG config file does not exist.", path);
        }

        return new AmneziaWGConfigDocument(File.ReadAllLines(path).ToList());
    }

    public void UpsertPeer(AmneziaWGPeer peer)
    {
